...
function Turnfunc ( {canvas, ctx, context, image, updateURL, getData_turn, setSelectTurn, setClickTurn, getImageUrl} ) {
    ...
    const onClickimageLeftRotate = () => {
        ...
        { rotate ?  drawimageRotate_h(context, image, -90) : drawimageRotate_v(context, image, -90)};
        ...
        const turnImg = canvasId.toDataURL('image/png');
    ...
    const onClickEndTurn = () => { // 이미지 회전 할때마다 이미지 URL로 빼내면.. 워크보드까지 같이 캡쳐되는 문제가 아직 해결되지 않아서 무한정 사진이 작아지는... 무한함수같은 문제가 생기기때문에.. 종료버튼 누를때의 상태만 캡쳐되도록 종료버튼 클릭시 함수에 toDataURL함수 사용
        ...
        context.rotate(rCnt * Math.PI/180);
    ...
    function drawimageRotate_h(context, image, degrees) {
        console.log('drawimageRotate_h');
        var filterType = context.filter; // 여기서 현재필터 인식하고 밑에서 회전 후 다시 씌워주는데 paint 쓰고 오면 얘도 인식이 안됨
        ...
        context.filter = 'none';
        context.clearRect(0 ,0, document.getElementById('canvasID').width, document.getElementById('canvasID').height);
        context.translate(canvasArea.width/2, canvasArea.height/2); 
        context.rotate(degrees * Math.PI/180);
        context.translate(-canvasArea.width/2, -canvasArea.height/2); // 회전후 - 붙여서 회전 축 다시 리셋해줘야 동일한 축 중심으로 회전 계속 함
    ...
    function drawimageRotate_v( context, image, degrees) {
